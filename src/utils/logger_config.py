...
# cache so we don't re-create handlers per module
...
class _ContextFilter(logging.Filter):
    ...
    Values can also be supplied via env vars (nice for orchestrators):
        RUN_ID, BATCH_ID
    """
    def __init__(self, module_name: str, run_id: Optional[str] = None, batch_id: Optional[str] = None):
        super().__init__()
...
def get_logger(
    ...
    Args:
        module_name: Short name of the pipeline stage (e.g., "extract", "transform", "load")
    ...
    logger.addFilter(_ContextFilter(module_name))
...
def get_run_log_dir(
    ...
    base_dir = base_dir or os.getenv("LOG_BASE_DIR", "logs")
    d = run_date or date.today()
    return Path(base_dir) / d.strftime("%Y-%m-%d")
